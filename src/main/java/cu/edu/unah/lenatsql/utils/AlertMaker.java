...
public class AlertMaker {
    ...
    public static void showMaterialDialog(StackPane root, Node nodeToBeBlurred, List<JFXButton> controls, String header, String body) {
        ...
        controls.forEach(controlButton -> {
            ...
            controlButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent mouseEvent) -> {
                copyToClipboardAndShowMsg(body);
                dialog.close();
            });
        });
    ...
    public static void showMaterialDialog(StackPane root, Node nodeToBeBlurred, Scene scene, List<JFXButton> controls, String header, String body) {
        BoxBlur blur = new BoxBlur(3, 3, 3);
        if (controls.isEmpty()) {
            controls.add(new JFXButton("Okay"));
        }
...
//        LibraryAssistantUtil.setStageIcon(stage);
        ...
        dialogPane.getStylesheets().add(AlertMaker.class.getResource("/resources/css/style.css").toExternalForm());
        dialogPane.getStyleClass().add("custom-alert");
    }

    private static void copyToClipboardAndShowMsg(String body){
